...
int base64_decoded_length(const char *input, size_t input_length,
  ...
  for (size_t i = 0; i < input_length; i++) {
    if (!is_base64_char(input[i], url_safe))
  ...
  if (input_length % 4 != 0) {
    if (!url_safe)
      return -1; // Non-URL-safe must have padding
...
#endif
    uint32_t sextet_a = (i < input_length && input[i] != '=')
                            ? decoding_table[(unsigned char)input[i++]]
                            : 0;
    uint32_t sextet_b = (i < input_length && input[i] != '=')
                            ? decoding_table[(unsigned char)input[i++]]
                            : 0;
    uint32_t sextet_c = (i < input_length && input[i] != '=')
                            ? decoding_table[(unsigned char)input[i++]]
                            : 0;
    uint32_t sextet_d = (i < input_length && input[i] != '=')
                            ? decoding_table[(unsigned char)input[i++]]
                            : 0;
